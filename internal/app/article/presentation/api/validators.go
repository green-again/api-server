...
func (v GetArticleRequestValidator) Validate(requestID interface{}) error {
	...
	if !ok {
		return fmt.Errorf("%w: %s", http.InvalidRequestError, fmt.Sprintf("request id %s is not string.", requestID))
	}
	_, err := uuid.Parse(val)
	if err != nil {
		return fmt.Errorf("%w: %s", http.InvalidRequestError, fmt.Sprintf("request id %s is not uuid format.", requestID))
	}
	return nil
}
